    ...
    <main className="relative min-h-screen overflow-hidden bg-[var(--color-surface)]">
      ...
      <div className="relative mx-auto grid min-h-screen w-full max-w-6xl gap-10 px-6 py-8 lg:grid-cols-[1fr_460px] lg:px-8">
        <section className="hidden lg:flex lg:flex-col lg:justify-between">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full border border-[color:var(--color-outline-variant)]/50 bg-white/70 px-4 py-2 text-[12px] font-medium text-[var(--color-on-surface)] backdrop-blur">
              <Sparkle size={16} weight="fill" className="text-[var(--color-primary)]" />
              IELTS Scholar Studio
            </p>
            <h1
              className="mt-8 max-w-xl text-5xl font-semibold leading-tight text-[var(--color-on-surface)]"
              style={{ fontFamily: "var(--font-display)" }}
            >
              Master IELTS with precision, not guesswork.
            </h1>
            <p className="mt-5 max-w-xl text-base leading-7 text-[var(--color-on-surface-variant)]">
              Analyze writing and speaking responses, save key vocabulary, and get context-aware
              explanations in one focused workspace.
            </p>
          </div>

          <div className="grid max-w-xl gap-3 rounded-3xl border border-[color:var(--color-outline-variant)]/35 bg-white/85 p-5 backdrop-blur">
            {[
              ...
              <div key={item} className="flex items-start gap-3">
                <CheckCircle size={20} weight="fill" className="mt-0.5 text-[var(--color-primary)]" />
                <p className="text-sm font-medium text-[var(--color-on-surface)]">{item}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="flex items-center lg:justify-end">
          <div className="w-full rounded-[2rem] border border-[color:var(--color-outline-variant)]/35 bg-white/90 p-6 shadow-[0_30px_90px_rgba(0,69,50,0.12)] backdrop-blur sm:p-8">
            <div className="mb-7">
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-on-surface-variant)]/80">
                {mode === "login" ? "Sign In" : "Sign Up"}
              </p>
              <h2
                className="mt-2 text-3xl font-semibold text-[var(--color-on-surface)]"
                style={{ fontFamily: "var(--font-display)" }}
              >
                {content.title}
              </h2>
            ...
            <form className="space-y-4" onSubmit={onSubmit}>
              <label className="block space-y-1.5">
                ...
                <Input
                  ...
                  {...form.register("email")}
                  className="h-12 rounded-2xl border-[color:var(--color-outline-variant)]/50 bg-[var(--color-surface-container-lowest)] px-4 text-sm"
                />
              ...
              <Button
                type="submit"
                disabled={formLoading}
                className="h-12 w-full rounded-2xl bg-[var(--color-primary)] text-sm font-semibold text-white hover:bg-[var(--color-primary-fixed-variant)]"
              >
            ...
            <div className="mt-5 flex items-center justify-between gap-3 text-xs text-[var(--color-on-surface-variant)]">
              ...
              <p className="font-medium">
                {content.altLabel}{" "}
                <Link className="text-[var(--color-primary)] hover:underline" href={content.altHref}>
                  {content.altText}
                </Link>
              </p>
            </div>
            ...
            <div className="mt-6">
              <Button
                ...
                className="h-11 w-full rounded-2xl border-[color:var(--color-outline-variant)]/60 bg-white text-sm font-medium"
              >
